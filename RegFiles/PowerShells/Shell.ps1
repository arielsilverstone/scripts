...
Start-Job { 
	Start-ThreadJob {	
		ForEach ($MSFT in $MSFT20) { 
			$Path = "HKCR:\WMP11.AssocFile."$($MSFT)"\shell\Enqueue"
			DEL -Path $Path  -Recurse 
		}	
	}				
	Start-ThreadJob { 	
		ForEach ($MSFT in $MSFT21) { 
			$PathY = "HKCR:\"+$MSFT+"\shell\Enqueue"
			$PathX = "HKCR:\"+$MSFT+"\shell\Play"		
...
Start-ThreadJob { 
	1..7 | ForEach {
		$Test="HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\ShellIconOverlayIdentifiers\ OneDrive"+$_
		If (Test-Path $($Test)) { 	
			Start-ThreadJob -InputObject $Test { 
...
$MSFT42 = 	"202913","202914","280797","280812","280813","280814","280815","310091","310092",`
			"310093","310094","314558","314559","314562","314563","314566","314567","338380",`
			"338381","338382","338386","338387","338388","338389","338393","353698","346480",`
			"346481","353694","353695","353696","353697","353698","353699",`
			"88000044","88000045","88000105","88000106","88000163","88000164","88000165",`
			"88000166","88000326"
Start-Job -ArgumentList @($MSFT40,$MSFT41,$MSFT42) {
	Param ($MSFT40,$MSFT41,$MSFT42)
	Start-ThreadJob -ArgumentList @($MSFT40) {	
		Param ($MSFT40)
		...
		ForEach ($MSFT in $MSFT40.Keys) {
			...
			SP -Path $PathV -Name $MSFT -Value $MSFT.Value
		}
	} -Name "Shell Extension"
	#
	Start-ThreadJob -ArgumentList @($MSFT41) {	
		Param ($MSFT41)
...
Start-Job {
	...
	ForEach ($Root in $Roots) {
		Start-ThreadJob -ArgumentList @($Root,$Desktops) {
			Param ($Root,$Desktops)
			$Vars = ":\Software\Wow6432Node\",":\Software\"
			ForEach ($Var in $Vars) {
				$Path = $Root+$Var+"Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders"
				ForEach ($Desktop in $Desktops) {
					SP -Path $Path	    -Name $Desktop	 -Value "D:\Desktop"
					SP -Path $Path+$Var -Name "Personal" -Value "D:\Users\Ariel"
				}
			}
...
#
	Start-ThreadJob {
		...
		$Vars 		= "\Wow6432Node\","\"
		ForEach ($Var in $Vars) {
			$Path = "HKLM:\SOFTWARE"+$Var+"Microsoft\Windows\CurrentVersion\Explorer\Shell Folders"
			ForEach ($Desktop in $Desktops) {
				SP -Path $Path -Name $Desktop 	-Value "D:\Desktop"
...
$End 	 = "\ShellFolder"
$Values  = @{ 					
<# Documents Folder #>			"{d3162b92-9365-467a-956b-92703aca08af}"="";`
<# Downloads Folder #>			"{088e3905-0323-4b02-9826-5d99428e115f}"="";`
<# Pictures Folder 	#>			"{24ad3ad4-a569-4530-98e1-ab02f9417aa8}"="";`
<# Music Folder 	#>			"{3dfdf296-dbec-4fb4-81d1-6a3438bcf4de}"="";`
<# Favorites		#>			"{323CA680-C24D-4099-B94D-446DD2D7249E}"="2696937728";`
<# Quick Access		#>			"{679f85cb-0220-4080-b29b-5540cc05aab6}"="2689597440";`
<# Gallery 			#> 			"{e88865ea-0e1c-4e20-9aa6-edcd0212c87c}"=""; `
<# CLSID_MSGraphHomeFolder #> 	"{f874310e-b6b7-47dc-bc84-b9e6b38f5903}"="";`
<# remove libraries #>			"{031E4825-7B94-4dc3-B131-E946B44C8DD5}"="2962227469"}		
Start-ThreadJob {
	ForEach ($Middle in $Middles ) {
		ForEach ($Value in $Values.Keys) {
			$Path  = $Base+$Middle+$Name+$End
			$PathX = "HKCU:\Software\Classes\"+$Middle+$Value
...
$PathB = $PathA+"\Bags\AllFolders\Shell"
$PathC = $PathA+"\BagMRU"
$PathD = "HKLM:\Software\Microsoft\Shell\ActionCenter\Quick Actions"
$PathE = "HKLM:\Software\Microsoft\Windows\CurrentVersion\Shell\Update\Packages"
Start-ThreadJob {
	...
	$PathE = $PathB+"\{5C4F28B5-F869-4E84-8E60-F11DB97C5CC7}"		 <# Generic Folder #>
	...
	SP -Path $PathE -Name "UndockingDisabled" 			-Value 0 	 -Type DWord
...
$Items = <# Remove "Create New Video" #> "AppX43hnxtbyyps62jhe9sqpdzxn1790zetc"+$Tail,`
		 <# and also 				  #> "AppXk0g4vb8gvt7b93tg50ybcy892pge6jmt"+$Tail
Start-ThreadJob {
...
$Hex	=	"([byte[]](0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xd1,0x4e,0x74,0xec,0x3e,
					   0x4a,0xd9,0x01))"
					   
$PairsP	=	"{E37E2028-CE1A-4F42-AF05-6CEABC4E5D75}"={"000214FA-0000-0000-C000-000000000046};`
			"0xFFFF"=$Hex;`
			<# Search Folder" #>
			"{04731B67-D933-450A-90E6-4ACD2E9408FE}"="-"
Start-ThreadJob {
	ForEach ($Item in $PairsA.Keys) {
		SP -Path $PathA -Name $Item -Value $Item.Value
	}
	ForEach ($Item in $PairsP.Keys) {
		SP -Path $PathP -Name $Item -Value $Item.Value
	}
} -Name "Approved Exts"
#
## Start Menu
...
Start-ThreadJob {
	SP -Path $Path -Name "CloseDwellTimeout"			-Value 300	-Type DWord
	SP -Path $Path -Name "MouseCloseThresholdPercent"	-Value 80	-Type DWord
	SP -Path $Path -Name "TouchCloseThresholdPercent"	-Value 80 	-Type DWord
} -Name "Shell switcher"
#
##
#
Start-ThreadJob {
	...
	NIP -Path $Path -Name HideRecentlyAddedApps 		-Value 1 	-Type DWord -Force
